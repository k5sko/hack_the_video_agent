...
def _teaches_cache() -> Path:
    path = HERE.parent / "data" / "cache" / "prune_teaches.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    return path


def verify_teaches(explains: List[dict], clips: Dict[str, dict]) -> Dict[str, bool]:
    """For each EXPLAINS claim, does the chapter TEACH the concept or merely use it?

    The extractor cannot tell the difference, and the distinction is the whole
    product. The single clip claiming to explain 'linear algebra' describes
    itself as one that "formalizes the neural network's operation *using* linear
    algebra" -- it applies the idea to teach something else, and a learner who
    does not already know linear algebra learns none of it there. Sending them
    that clip is worse than admitting the corpus never covers it, because it
    looks like an answer.

    No score threshold separates the two: a real explanation and a competent
    application both land around 0.75-0.85, which is exactly where that clip sat.
    So the claim gets checked against the chapter's own summary.

    Returns {"clip_id||concept": teaches?}. Cached, so this is a one-off cost.
    """
    cache_file = _teaches_cache()
    verdicts: Dict[str, bool] = {}
    if cache_file.exists():
        try:
            verdicts = json.loads(cache_file.read_text())
        except json.JSONDecodeError:
            verdicts = {}

    def key(clip_id: str, concept: str) -> str:
        return f"{clip_id}||{concept}"

    pending = [
        e
        for e in explains
        if key(e["clip_id"], e["concept"]) not in verdicts
        and (clips.get(e["clip_id"], {}).get("summary"))
    ]
    if pending:
        sys.path.insert(0, str(HERE.parent / "graph"))
        ...
        for i in range(0, len(pending), 40):
            ...
            listing = [
                {
                    "i": j,
                    "concept": e["concept"],
                    "chapter_summary": clips[e["clip_id"]]["summary"][:600],
                }
            ...
            try:
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": (
                                "Each item pairs a concept with the summary of a "
                                "lecture chapter that supposedly explains it.\n\n"
                                "Decide, for each, whether someone who did NOT "
                                "already know that concept would actually LEARN it "
                                "from this chapter.\n\n"
                                "YES if the chapter introduces, defines, derives, "
                                "or walks through how the concept works -- even "
                                "briefly, and even while building toward something "
                                "else. Example: 'explains how value vectors are "
                                "generated by multiplying embeddings with a value "
                                "matrix' does teach value vectors.\n\n"
                                "NO only when the concept is background the chapter "
                                "takes for granted, used as a tool to explain a "
                                "different topic. Example: 'formalizes the network "
                                "using linear algebra' does not teach linear "
                                "algebra -- a learner who did not already know it "
                                "would be lost.\n\n"
                                "When genuinely unsure, answer yes.\n\n"
                                f"{json.dumps(listing, indent=1)}\n\n"
                                'Return JSON {"teaches": [i, ...]} listing only the '
                                "indices that genuinely teach their concept."
                            ),
                        }
                    ],
                ...
                taught = set(
                    json.loads(response.choices[0].message.content).get("teaches", [])
                )
            except Exception as exc:
                print(
                    f"  teaches-verification failed ({exc}); keeping claims as-is",
                    file=sys.stderr,
                )
                taught = set(range(len(batch)))
            for j, e in enumerate(batch):
                verdicts[key(e["clip_id"], e["concept"])] = j in taught
        cache_file.write_text(json.dumps(verdicts, indent=1, sort_keys=True))
...
def prune(
    ...
    # 2b. Drop EXPLAINS claims where the chapter only uses the concept. A clip
    #     that applies an idea to teach something else does not teach the idea,
    #     and offering it as coverage is worse than reporting a gap -- a gap is
    #     honest, a wrong clip looks like an answer.
    demoted = 0
    if use_embeddings:  # same switch as the other model-backed pass
        clips_by_id = {c["id"]: c for c in p.get("clips", [])}
        verdicts = verify_teaches(p["explains"], clips_by_id)
        kept = []
        for e in p["explains"]:
            verdict = verdicts.get(f"{e['clip_id']}||{e['concept']}")
            if verdict is False:
                demoted += 1
                continue
            kept.append(e)
...
def main() -> int:
    ...
    print(f"  EXPLAINS demoted to mentions: {stats['demoted_explains']}")
